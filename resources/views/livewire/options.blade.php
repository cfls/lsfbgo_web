<div class="space-y-4 min-h-screen">
    @php
        ...
        if ($syllabusData && isset($syllabusData['attributes'])) {
            ...
            $bgClass = $attributes['hex_color'] ?? '#e5e7eb';
        }
    @endphp
    ...
    <div class="bg-gray-300 p-5 w-full rounded-lg space-y-5 flex flex-col items-center justify-start
        ...
        @foreach($this->themes as $index => $theme)
            @php
                ...
                $colorHex  = $alreadyDone ? '#519A66' : $bgClass;
                $textColor = textColorForBg($colorHex);
            ...
            <a href="{{ $link }}" class="w-full max-w-sm">
                <div class="flex items-center justify-between p-5 border border-gray-200 rounded-lg shadow-sm transition duration-300"
                     style="background-color: {{ $colorHex }}; color: {{ $textColor }};">
                    <div class="flex items-center gap-2">
                        <flux:label class="text-lg font-semibold" style="color: {{ $textColor }};">
                    ...
                    @if($locked)
                        {{-- 🔒 Bloqueado --}}
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6"  style="color: {{ $textColor }};">
